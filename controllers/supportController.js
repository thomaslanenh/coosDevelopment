...
exports.messagedetail = function (req, res, next) {
  db.tx(async (t) => {
    ...
    const messageloop = await t.manyOrNone(
      'SELECT * from messages m INNER JOIN useraccount u on m.sender_id = u.id INNER JOIN usertypes ut on u.user_type = ut.type_ref WHERE master_id = $1',
      [req.params.messageid]
    );
